...
- 灵衢通算解决方案架构，基于最小依赖和可靠性的考虑，UBSE默认通信选择UB(URMA)；

- UBSE提供TCP+TLS的安全建链方式，消减仿冒攻击、信息泄漏等安全风险，供客户选择；

**用户侧的安全配置：**

- 用户在UBSE的配置文件中，切换UB和TCP通信

- 在TCP通信模式下，用户需要额外导入身份证书，作为TLS协议的证书凭据
...
- 当前安全策略：受LCNE仅支持TCP+HTTP、不支持HTTPS的约束，当前UBSE与LCNE间通信采用本地回环通信，存在仿冒、消息篡改等威胁，需要客户保证UBSE运行的软硬件环境是可信的；
- 后续安全策略： 待LCNE实现UDS通信能力后，UBSE与LCNE间通信采用UDS通信机制，通过uds的sock文件权限控制来达成安全访问.
...
- 默认通信UB(URMA)，没有认证和加密，存在仿冒、消息泄漏等安全风险，需要客户保证UBSE运行的软硬件环境是可信的
- 受LCNE仅支持TCP+HTTP、不支持HTTPS的约束，当前UBSE与LCNE间通信采用本地回环通信，存在仿冒、消息篡改等威胁，需要客户保证UBSE运行的软硬件环境是可信的；
