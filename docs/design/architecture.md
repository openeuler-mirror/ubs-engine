...
- **ubse-daemon：**ubse工作主进程，提供ubse的资源管理核心功能
  ...
  - **adv controllers：**负责提供各种与业务强相关的资源管理能力，如NPU、DPU联合调
...
实现对BMC下单命令劫持。该命令劫持时，客户业务暂时未受影响，因此当迁移失败时，可以定位完后，确认回迁后再进行重新下电。
...
**①自动拓扑发现：**通过LCNE的拓扑发现机制，获取全量节点连接信息
...
- **周期对账：**对账只完成master、agent间的mem controller数据对账，不与OBMM进行周期对账；对账周期为5分钟，不可配置；周期对账时，将该节点集群状态迁移到Smooting状态，通知mem controller、mem scheduler，禁止新业务接入，从而实现并发控制
