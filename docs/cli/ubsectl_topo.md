...
1. 当前只支持CPU类型节点的连线关系
2. 只能获取到当前网络分区内的节点的链接信息。
3. CLOS组网下不支持获取cpu拓扑信息

### 输出信息说明

1. 规格说明：
   a. 表内每一行表示两个CPU设备之间的链接信息
   b. 表内列信息说明如下

| 表头                  | 说明                                                                                                                                                | 取值 
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------| ----------
| link-id             | 表示一条链接的ID。<br>1.当本端节点的MXE尚未被划入本节点MXE所在的网络平面时，显示“-”，表示该链路不可用                                                                                       | 字符串
| node                | 排序较小一端的节点信息。例：computer1(1)，由2部分组成：<br>1.括号前部分：主机名为computer1；<br>2.括号内部分：节点在MatrixServer中的槽位号，为1;<br>3.当本端节点的MXE尚未被划入本节点MXE所在的网络平面时，显示“-”，表示缺乏信息   | 字符串
| socket              | 排序较小一端的节点信息内，OS上标识某一个CPU的socketId值，例：36<br>1.当该节点的MXE上报信息失败时，显示“-”来表示缺乏信息                                                                         | 整数
| port                | 排序较小一端的节点信息内，CPU硬件上的端口在MatrixServer中的端口值，例：0 <br>1.当该节点的MXE上报信息失败时，显示“-”来表示缺乏信息                                                                   | 整数
| interface-name      | 排序较小一端的端口接口名称。例：400GUB1/1/1。<br>1.当无法获取到接口名称时，显示"-"来表示缺乏信息；<br>2.此名称与UBM配置命令、告警、性能指标中的端口名称对应       | 字符串
| peer-node           | 排序较大一端的节点信息。例：computer2(2)，由2部分组成：<br>1.括号前部分：主机名为computer2；<br>2.括号内部分：节点在MatrixServer中的槽位号，为2; <br>3.当对端节点的MXE尚未被划入本节点MXE所在的网络平面时，显示“-”，表示该链路不可用 | 字符串
| peer-socket         | 排序较大一端的节点内，OS上标识某一个CPU的socketId值，例：36 <br>1.当该节点的MXE上报信息失败时，显示“-”来表示缺乏信息                                                                          | 整数
| peer-port           | 排序较大一端的节点内，CPU硬件上的端口在MatrixServer中的端口值，例：0 <br>1.当该节点的MXE上报信息失败时，显示“-”来表示缺乏信息                                                                     | 整数
| peer-interface-name | 排序较大一端的端口接口名称。例：400GUB2/1/1。<br>1.当无法获取到接口名称时，显示"-"来表示缺乏信息；<br>2.此名称与UBM配置命令、告警、性能指标中的端口名称对应       | 字符串

c. 链接对显示完成后，最后两行会显示当前网络平面的已知总链接数：**Total Links**，和可用链接数：**Available Links**。

2. 注意事项
   a. 显示的链接对不重复，按照**本端节点在MatrixServer中的槽位号+OS上标识某一个CPU的socketId值+该端口在MatrixServer中的端口值**，排序显示。
   b. LinkId和对端设备信息显示“-”时，表示本端设备上该端口存在一个已知连线，但未知对端的具体信息，原因可能为对端节点不在当前网络分区内。此时无法正常借用远端内存，因此表示为-。
...
当某一节点不在当前集群时，显示为“-”。具体如下，以缺少2节点信息为例。
...
2. 注意事项
   ...
   d. 错误处理逻辑与集群其他命令保持一致
   e. 如需查询多个节点，请使用 ubsectl display cluster命令
...
- 流程图
  ![img.png](../images/img-ubse-node2.png)
