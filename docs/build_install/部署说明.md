# 部署说明

## 1 概述

ubs engine的发布件包含：开发包、运行包

ubs engine的部署分为开发包部署和运行包部署

- 开发包：主要用于支持开发者使用ubs engine提供的接口，进行业务开发

- 运行包：主要用于在生产环境中与周边ubfm等模块交互，提供ub池化资源管理能力

ubs engine的发布件，由以下三个 RPM 包组成：

| RPM包                                                   | 说明                                   |
| ------------------------------------------------------- | -------------------------------------- |
| ubs-engine-<version>-<release>.aarch64.rpm              | 主程序包，包含服务、CLI、配置等        |
| ubs-engine-client-libs-<version>-<release>.aarch64.rpm  | 客户端运行时库（供第三方程序动态链接） |
| ubs-engine-client-devel-<version>-<release>.aarch64.rpm | 开发包（含头文件与静态库）             |

## 2 安装开发包

### 2.1 环境要求

业务开发环境中部署即可，无特殊要求

### 2.2 安装命令

```bash
# 安装开发包（用于集成开发）
sudo dnf install -y ubs-engine-client-devel-1.0.0-1.aarch64.rpm
```

### 2.3 安装结果

| 文件/目录                      | 其它说明                                     |
| ------------------------------ | -------------------------------------------- |
| `/usr/include/ubse`            | 目录下头文件（`*.h`）权限：`644`             |
| `/usr/lib64/libubse-client.so` | 软链接，指向`/usr/lib64/libubse-client.so.1` |
| `/usr/lib64/libubse-client.a`  | 二进制静态库                                 |

> 完整权限说明见《安全设计》文档

## 3 安装运行包

### 3.1 系统要求

* **操作系统：** openEuler 24.03 LTS SP3 或更高版本
* **CPU架构：** aarch64
* **内存：** ≥ 64GB
* **磁盘：** SSD，IOPS 500MB/s
* **芯片互联：** UB
* **网卡：**可选依赖（可选使用TCP辅助UB建链，默认采用UB自举建链）
* **用户权限：** 安装与管理需 `root` 权限

### 3.2 节点规划

* 所有集群节点均需安装 `ubs-engine` 主包。

* 建议提前规划节点 IP 地址列表（用于 TCP 模式）

### 3.3 依赖运行时库

运行依赖以下系统库，通常由包管理器自动安装：

* `glibc`, `libstdc++`, `libgcc`

* `libxml2`

* `numactl-libs`

* `libboundscheck`

* `ubs-hcom`

* `systemd`

### 3.4 RPM 包安装步骤

```bash
# 安装主程序包
sudo dnf install -y ubs-engine-1.0.0-1.aarch64.rpm

# 安装客户端运行时库（第三方集成必需）
sudo dnf install -y ubs-engine-client-libs-1.0.0-1.aarch64.rpm
```

> 若使用 `rpm` 命令，请注意依赖顺序：
> ```bash
> sudo rpm -ivh ubs-engine-1.0.0-1.aarch64.rpm
> sudo rpm -ivh ubs-engine-client-libs-1.0.0-1.aarch64.rpm
> ```

### 3.5 安装结果

- ubs-engine 主程序安装结果：

| 路径                                   | 用途         |
|--------------------------------------|------------|
| /usr/bin/ubse, /usr/bin/ubsectl      | 主程序与 CLI   |
| /usr/lib/systemd/system/ubse.service | systemd 服务 |
| /etc/ubse/                           | 配置目录       |
| /var/log/ubse/                       | 日志目录       |
| /var/lib/ubse/                       | 持久化数据      |
| /var/lib/ubse/cert/                  | 证书目录       |
| /var/run/ubse/                       | 运行时 socket |

> 完整权限说明见《安全设计》文档

- ubs-engine 客户端运行时库安装结果：

| 文件                                   | 其它说明                                        |
|--------------------------------------|---------------------------------------------|
| `/usr/lib64/libubse-client.so.1.0.0` | 二进制动态库实体                                    |
| `/usr/lib64/libubse-client.so.1`     | 软链接，指向 `/usr/lib64/libubse-client.so.1.0.0` |

> 完整权限说明见《安全设计》文档

## 4 通信模式选择：URMA vs TCP

ubs engine支持两种通信模式，可根据硬件和网络环境选择：

| 通信方式   | URMA(默认)             | TCP                    |
| ---------- | ---------------------- | ---------------------- |
| 性能       | 高性能、低延迟、零拷贝 | 标准 TCP，性能适中     |
| 硬件要求   | 需支持 URMA 的智能网卡 | 普通以太网卡即可       |
| 配置复杂度 | 免配置，自动发现节点   | 需手动配置 IP 列表     |
| 使用场景   | 高性能计算、金融交易   | 普通数据中心、开发测试 |

### 4.1 URMA通信模式（推荐）

ubs engine采用urma通信方式，有如下特点：

* 安装后直接启动服务，无需任何配置。

* 节点通过 URMA 自动发现并建立连接。

* 适用于具备 URMA 硬件支持的环境。

#### 4.1.1 安装URMA

确保已安装 URMA 驱动和运行时库，否则服务将无法启动或通信失败

```bash
# 示例：安装 URMA 运行时包（具体包名根据发行版可能不同）
sudo yum install -y umdk-urma-lib    # OpenEuler
sudo yum install -y umdk-urma-kmod
```

#### 4.1.2 启动ubs engine服务

```bash
sudo systemctl start ubse
sudo systemctl enable ubse
```

### 4.2 TCP通信模式

#### 4.2.1 修改配置

1. 编辑配置文件：

```bash
sudo vi /etc/ubse/ubse.conf
```

2. 修改以下配置项(默认无此配置项,打开此配置时，使用tcp通信，否则默认使用urma通信)：

```ini
[ubse.rpc]
cluster.ipList=192.168.100.100-192.168.100.102
```

> 支持 配置IP 范围：192.168.100.100-192.168.100.102

#### 4.2.2 重启ubs engine服务

```bash
sudo systemctl restart ubse
```

## 5 ubs engine服务管理

### 5.1 启动服务

```bash
sudo systemctl start ubse
```

### 5.2 停止服务

```bash
sudo systemctl stop ubse
```

### 5.3 查看状态

```bash
sudo systemctl status ubse
```

### 5.4 开机自启

```bash
sudo systemctl enable ubse
```

## 6 验证部署

### 6.1 检查服务状态

```bash
systemctl is-active ubse  # 应输出 "active"
```

### 6.2 查看日志

```bash
#方式一：
journalctl -u ubse -f

#方式二：参看日志文件
/var/log/ubse/xxx
```

### 6.3 使用 CLI 工具

```bash
sudo -u ubse /usr/bin/ubsectl --help
```

## 7 升级与卸载

### 7.1 升级

```bash
sudo dnf update -y ubs-engine*.rpm
# 或
sudo rpm -Uvh ubs-engine*.rpm
```

### 7.2 卸载

```bash
sudo dnf remove -y ubs-engine*
# 或
sudo rpm -e ubs-engine-client-devel ubs-engine-client-libs ubs-engine
```

> 卸载不会删除 `/var/lib/ubse/` 和 `/var/log/ubse/` 中的数据，如需清理请手动删除。

## 8. 常见问题

### Q1: 服务启动失败，提示权限错误？

* 检查 `/var/log/ubse` 和 `/var/lib/ubse` 所属用户是否为 `ubse:ubse`。

* 使用 `chown -R ubse:ubse /var/lib/ubse /var/log/ubse` 修复。

### Q2: CLI 命令补全无效？

* 安装 `bash-completion`：`sudo dnf install bash-completion`

* 重新登录或执行：`source /etc/bash_completion.d/cli_commands.sh`

### Q3: 如何查看当前通信模式？

* 使用命令快速查看配置：

```bash
grep -E "cluster.ipList" /etc/ubse/ubse.conf
```

输出示例:

```text
# cluster.ipList=192.168.100.100-192.168.100.102,192.168.100.104
```

* 若显示 cluster.ipList 行首有 # 注释符 , 当前使用 URMA 模式
* 若显示 cluster.ipList 行首无 # 注释符, 当前使用 TCP 模式
