...
当节点被攻破时，已生成的导出内存可能被非法节点导入，为解决该问题，设计了高安方案
使用UBSE进行普通借用时，导出的内存，只能在申请的节点导入;  共享借用时，可以被多个节点导入
在高安场景，ubse将运行在物理机，部分相关组件将运行在机密虚机里，UBSE通过https + tls，vsock + tls的方式与虚机内的相关组件进行交互，从而实现高安目的
...
Cli导入命令示例：
```
ubsectl import cert -c {path}/<ca-cert-file> -s {path}/<server-cert-file> -k {path}/<server-key-file> -l {path}/<ca-crl-file>

path值为环境上的证书路径，该cli命令会将该路径下的证书拷贝到/var/lib/ubse/cert/目录下，并修改对应权限
```
