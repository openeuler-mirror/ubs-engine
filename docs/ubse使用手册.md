...
UBSE（UB Service Core Engine, 下文简称UBSE或UBS Engine）提供了ubse daemon程序及其相应的SDK开发库。开发者可以利用该SDK访问ubse daemon提供的服务，从而实现对MEM（内存）等资源的调度与管理，执行关键运维操作。
...
UBSE软件核心能力如下图所示
![1773057592643](image/ubse使用手册/1773057592643.png)

### 1.3 项目代码架构

UBSE软件代码架构如下图所示
![1773057687974](image/ubse使用手册/1773057687974.png)
...
#### 3.1.1 ubs_urma_dev_get:查询独占/共享urma设备信息
 ...
 */
int32_t ubs_urma_dev_get(ubs_urma_type urma_type, urma_device_t **urma_devices, uint32_t *urma_cnt);
```
- **数据结构**
...
| :--- | :--- | :--- |
| urma_devices | OUT | 所有对应类型的urma设备信息 |
| urma_cnt |OUT | 所有urma设备数量 |

#### 3.1.2 ubs_urma_dev_alloc:分配urma设备
...
| **字段名** | **字段描述** | **字段取值** |
| :--- | :--- | :--- |
| --node <br>-n | 节点id（和slotid对应）<br>字段可选，不传默认查询本节点 | 1~max_id |
...
```bash
git clone -b br_container_dev_20260130 https://gitee.com/openeuler/ubs-engine.git //目前构建分支为br_container_dev_20260130
```

#### 4.1.2 安装依赖

- **方式一 DNF一键安装spec中指定的BuildRequires**
...
```bash
yum install -y libboundscheck ubs-hcom-devel libxml2-devel numactl-libs
```
...
##### 5.2.2.2 ubs-engine-client-libs
...
##### 5.2.2.3 ubs-engine-client-devel
...
| [ubse.ubfm] | lcne.server.port | 向LCNE进程发送消息的端口。 | 如果UBM部署到高安环境，开启该配置项，配置UBSE与LCNE的通信端口，取值范围：[1024,65535]<br>参数配置取值范围之外或错误值都会被重置为34256。<br>默认关闭该配置，UBSE与UBM之间节点内采用UDS通信。 | 所有节点 |
...
#Set the Port for Sending Messages to the lcne. The value range is [1024, 65535]. Any value out of range is reset to 34256
The value must match the lcne listening port.
lcne.server.port=34256
