...
| 序号 | 参数             | 说明                                             | 取值                                                         |
| ---- | ---------------- | ------------------------------------------------ | ------------------------------------------------------------ |
| 1    | ubse.server.port | UBSE暴露给UBM的通信端口，在该端口开启HTTP服务。 | 默认值：8082<br>取值范围：[1024,65535]<br>参数配置取值范围之外或错误值都会被重置为8082。 |
| 2    | ubm.server.port | UBM暴露的通信端口，UBSE通过该端口与UBM交互     | 默认值：34256<br>取值范围：[1024, 65535]<br>参数配置取值范围之外或错误值都会被重置为34256。 |
