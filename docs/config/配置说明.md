...
| 2    | cluster.ipList  | rpc使用tcp协议时的集群各节点ip地址 | 默认值：192.168.100.100-192.168.100.103。<br>打开此配置时，使用tcp通信，此配置注释时使用urma通信。<br>参考值：192.168.100.100-192.168.100.103 |
| 3    | cert.use        | rpc安全证书能力开关（仅TCP支持）   | 默认值：true（开启证书能力）<br>取值范围：[true，false]。<br>如果取值超过范围，则取默认值true。 |
