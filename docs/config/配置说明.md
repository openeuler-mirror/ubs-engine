...
| 序号 | 参数      | 说明                                                   | 取值                                                         |
|------|-----------|--------------------------------------------------------|--------------------------------------------------------------|
| 1    | topo_mode | UBSE 在 CLOS 组网下向 URMA UVS 下发拓扑时使用的建链模式。1D-FULLMESH 组网下该配置不生效，默认注释。 | 默认值：non-cross<br>取值范围：<br>- non-cross<br>- hccs-cross<br>参数配置取值范围之外或错误值都会被重置为 non-cross。 |
