...
| 序号 | 参数             | 说明                                             | 取   值                                                                                                                    |
| ---- | ---------------- | ------------------------------------------------ |-----------------------------------------------------------------------------------------------------------------------|
| 1    | ubse.server.port | 当ubm.server.cid配置项启用时，该配置项生效，UBSE暴露给UBM的通信端口，在该端口开启HTTPS服务。 | 默认关闭该配置，UBSE与UBM之间节点内采用UDS通信。<br>当ubm.server.cid配置开启时才生效，取值范围：[1024, 65535]，如果配置非法值会被重置为默认值8082 |
| 2    | ubm.server.port | 当ubm.server.cid配置项启用时，该配置项生效，UBM暴露的通信端口，UBSE通过该端口与UBM进行HTTPS交互。 | 默认关闭该配置，UBSE与UBM之间节点内采用UDS通信。<br>当ubm.server.cid配置开启时才生效，取值范围：[1024, 65535]，如果配置非法值会被重置为默认值8799 |
| 3    | ubm.server.cid | 此配置项作为高安模式的使能开关。<br>UBM和UBSE共部署在host主机时，该配置项关闭，两者之间使用UDS通信；<br>在高安部署模式下，该配置开启，此时UBM部署在机密虚机，UBSE与UBM之间采用vsock通信，本配置同时作为vsock的cid地址，取值范围：uint32_t；配置非法值，将导致与UBM通信失败，UBSE无法对外提供服务。 | 默认关闭该配置，UBSE与UBM之间节点内采用UDS通信。<br>开启该配置项，配置虚机启动时候使用的cid，取值：uint32_t，配置非法值，与虚机使用vsock通信会失败 |
