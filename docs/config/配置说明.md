...
| 2    | ubse.server.port | 当ubm.server.cid配置项启用时，该配置项生效，UBSE暴露给UBM的通信端口，在该端口开启HTTPS服务。 | 当ubm.server.cid配置开启时才生效，取值范围：[1024, 65535]，如果配置非法值会被重置为默认值8799 |
| 3    | ubm.server.port | 当ubm.server.cid配置项启用时，该配置项生效，UBM暴露的通信端口，UBSE通过该端口与UBM进行HTTPS交互。 | 当ubm.server.cid配置开启时才生效，取值范围：[1024, 65535]，如果配置非法值会被重置为默认值8082 |
