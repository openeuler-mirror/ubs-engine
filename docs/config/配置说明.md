...
| 9  | radius.lender      | 借出节点的内存借用半径。        | 默认值：7<br>取值范围：[0, 7]<br>如果取值超过范围，则取默认值7。                               | - 控制借出节点允许借用内存的半径范围。<br>- 所有节点的配置需保持一致。                                                                                                                                     |
| 10 | radius.borrow      | 借入节点的内存借用半径。        | 默认值：7<br>取值范围：[0, 7]<br>如果取值超过范围，则取默认值7。                               | - 控制借入节点请求内存的半径范围。<br>- 所有节点的配置需保持一致。                                                                                                                                     |
