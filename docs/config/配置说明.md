...
[ubse.urma]
# Topology mode used when UBSE pushes topology to URMA UVS in CLOS networking.
# Valid values: non-cross, hccs-cross. Any invalid value will be reset to non-cross.
topo_mode=non-cross
...
| 序号 | 参数             | 说明                                             | 取值                                                                                                                    |
| ---- | ---------------- | ------------------------------------------------ |-----------------------------------------------------------------------------------------------------------------------|
| 1    | ubse.server.port | UBSE暴露给UBM的通信端口，在该端口开启HTTP服务。 | 默认关闭该配置，UBSE与UBM之间节点内采用UDS通信。<br>如果UBM部署到高安环境，开启该配置项，配置UBSE与UBM的通信端口，取值范围：[1024, 65535]<br>参数配置取值范围之外或错误值都会被重置为8082。      |
| 2    | ubm.server.port | UBM暴露的通信端口，UBSE通过该端口与UBM交互     | 默认关闭该配置，UBSE与UBM之间节点内采用UDS通信。<br>如果UBM部署到高安环境，开启该配置项，配置UBSE与UBM的通信端口，取值范围：[1024, 65535]<br>参数配置取值范围之外或错误值都会被重置为34256。 |
| 3    | ubm.server.cid | 基于安全考虑，可以将UBM部署到机密虚机中，此时ubse与ubm之间采用vsock通信，本配置项表示ubm所在机密虚机的地址     | 默认关闭该配置，<br>开启该配置项，配置虚机启动时候使用的cid, 取值：uint32_t, 配置非法值，与虚机使用vsock通信会失败 <br> |
...
| 序号 | 参数      | 说明                                                   | 取值                                                         |
|------|-----------|--------------------------------------------------------|--------------------------------------------------------------|
| 1    | topo_mode | UBSE 在 CLOS 组网下向 URMA UVS 下发拓扑时使用的建链模式。1D-FULLMESH 组网下该配置不生效。 | 默认值：non-cross<br>取值范围：<br>- non-cross<br>- hccs-cross<br>参数配置取值范围之外或错误值都会被重置为 non-cross。 |
...
| 序号 | 参数               | 说明                              | 取值                                                | 备注 |
|------|--------------------|-----------------------------------|-----------------------------------------------------|------|
| 1    | system.pool.memory.ratio | 所有节点的池化内存比例。                    | 默认值：100<br>取值范围：[0, 100]<br>单位：%        | —    |
| 2    | obmm.memory.block.size   | 芯片表项内存拆分粒度大小                    | 默认值：128<br>取值范围：[2, 1024]<br>单位：MB<br>参数取值必须是2的倍数 | —    |
| 3    | isLender           | 当前节点是否可以作为内存借出节点。            | 默认值：true<br/>取值范围：<br/>- true<br/>- false  | —    |
| 4    | lender.balance     | 算法是否使用借出 NUMA 均衡模式。             | 默认值：false<br/>取值范围：<br/>- true<br/>- false | - **true**：资源充足时，不同的借用请求节点尽量优先分散到不同节点的借出numa或者相同节点的不同借出numa上，相同借用请求节点优先复用其历史使用的借出numa。<br>- **false**：默认决策方式，资源充足时，不同的借用请求节点也可能由相同的借出节点的借出numa借出（借出numa优先选择最大剩余内存的numa）|
