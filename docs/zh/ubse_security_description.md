...
- UBSE在TLS握手阶段内置证书自校验能力：解析本端身份证书（server.pem）中SAN扩展字段的otherName（OID 1.3.6.1.4.1.2011.999.1），将其与本节点nodeId比对，不一致则拒绝握手；对端证书链与CRL校验由mTLS基线保障，实现双向身份确认防仿冒。
...
ubs engine的运行账号是ubse，所属的用户组是ubse，用户无密码不可登录，用户和用户组在rpm包安装时创建。
