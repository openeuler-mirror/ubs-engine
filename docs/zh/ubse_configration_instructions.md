...
| 2    | vip.httpServer.listen.ip   | VIP 地址及 CIDR 前缀。开启 VIP 时该项必填。           | 默认值：用#注释<br>格式：`<ip>/<prefix>`，例如 `192.168.100.200/24`<br>prefix 取值范围：[1, 32]<br>非法 IP 或 prefix 超范围时启动失败。                                              | - 地址必须为合法 IPv4 地址。<br>- prefix 表示子网掩码长度。<br>- 生成证书时需在 subjectAltName 中指定访问标识：虚机场景添加该 VIP 的 IP SAN 条目（如 `IP = 192.168.100.200`）；容器场景添加 DNS SAN 条目（如`DNS = ubse-helper.ubse-system.svc.cluster.local`）                       |
