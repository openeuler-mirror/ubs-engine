...
| 6  | lender.balance     | 算法是否使用借出 NUMA 均衡模式。 | 默认值：false<br>取值范围：[true，false]<br>如果取值超过范围，则取默认值false。                     | - **true**：资源充足时，不同的借用请求节点尽量优先分散到不同节点的借出numa或者相同节点的不同借出numa上，相同借用请求节点优先复用其历史使用的借出numa。<br>- **false**：默认决策方式，资源充足时，不同的借用请求节点也可能由相同的借出节点的借出numa借出（借出numa优先选择最大剩余内存的numa）。<br>此配置项已弃用，请使用 scheduler.mode 替代。                                                                                                                                                                                                                                                                                                             |
