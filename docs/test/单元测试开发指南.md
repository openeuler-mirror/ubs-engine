...
> 背景假设：
>
> 开发者想针对当前的业务代码 `src/framework/misc/ubse_str_util.cpp`中的`ConvertStrToUint32` 函数添加测试用例，当前在`test/UT/utils/test_rack_str_util.cpp`已经存在部分用例
...
在`test/UT/utils/test_rack_str_util.cpp`中新增如下代码
...
>⚠️ 注意：只有当所有单元测试 **100% 通过**时，才允许生成覆盖率报告。
