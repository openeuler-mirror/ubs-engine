...
mockcpp 通过 `mprotect` 修改代码段页属性实现 API 打桩。项目中应用的 ARM64 支持补丁（`test/mockcpp/mockcpp_support_arm64.patch`）中地址对齐宏 `ADDR_ALIGN_UP`/`ADDR_ALIGN_DOWN` 按 4K 页硬编码。在 64K 内存页环境（如鲲鹏 920 默认配置）下，传入 `mprotect` 的地址未按实际页大小对齐导致调用失败，进程向只读代码段写入指令时崩溃（SIGSEGV）。
