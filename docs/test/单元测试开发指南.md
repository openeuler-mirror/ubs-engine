...
本指南旨在帮助开发者为 `ubs_engine` 项目编写高质量的 C++ 单元测试（Unit Test，UT）。内容包括整体测试框架、环境搭建、用例编写规范以及运行与覆盖率分析方法，确保代码质量可控、可维护。
...
>💡 提示：需预先安装 GTest/GMock（`gtest-devel gmock-devel`）。
...
在`ubs_engine`目录下对用例进行验证。

`TestRackStrUtil`为**`TestSuite`**名，`ConvertStrToUInt32_Valid`为**`TestName`**，执行以下命令：

```bash
bash build.sh ut -- --gtest_filter="TestRackStrUtil.ConvertStrToUInt32_Valid"
```
...
> [!NOTE] 说明
>
> 开发一个模块的测试代码时，只需要add_ut一次即可。在该目录添加模块的其它测试代码时，若不依赖其它库，不需要改动CMake文件。若依赖其它库则可以再添加target_link_libraries。
...
在`test/UT/log`目录下新建测试文件`test_ubse_logger.h`和`test_ubse_logger.cpp`，完成对具体的业务代码的测试，在`TestUbseLogger`件中写了一个测试用例。
...
在ubs_engine目录下查看用例运行情况。

```bash
bash build.sh ut -- --gtest_filter="TestUbseLogger.TestoperatorChar"
```
...
> [!NOTE] 说明
>
> `--gtest_filter` 支持通配符（`*`）、排除（`-`）等语法，详见 GTest 文档。

![image-20250924111444689](images/image-20250924111444689.png)

## 5 64K 内存页环境兼容性说明（mockcpp 限制与 UT 跳过策略）

### 5.1 问题背景

mockcpp 通过 `mprotect` 修改代码段页属性实现 API 打桩。项目中应用的 ARM64 支持补丁（`3rdparty/patch/mockcpp_support_arm64.patch`）中地址对齐宏 `ADDR_ALIGN_UP`/`ADDR_ALIGN_DOWN` 按 4K 页硬编码。在 64K 内存页环境（如鲲鹏 920 默认配置）下，传入 `mprotect` 的地址未按实际页大小对齐导致调用失败，进程向只读代码段写入指令时崩溃（SIGSEGV）。

### 5.2 跳过策略（不修改 mockcpp 框架）

UT 执行入口（`scripts/cmake/utils.cmake` 中 `add_ut`、`add_independent_exec_ut` 两个宏）在 CMake 配置阶段通过 `getconf PAGESIZE` 检测系统内存页大小；非 4K 页时打印说明信息并将 UT 执行命令替换为跳过提示（echo），构建流程正常退出，不影响 `bash build.sh ut` 等整体流程的执行。编译构建本身与内存页大小无关，仍正常执行。

### 5.3 跳过逻辑验证

如需在 4K 页环境验证跳过行为，可设置环境变量 `MOCKCPP_PAGE_SIZE_OVERRIDE`（如 `MOCKCPP_PAGE_SIZE_OVERRIDE=65536 bash build.sh ut`）强制按指定页大小判断；该变量仅用于验证跳过逻辑，请勿在正式环境使用。

### 5.4 直接执行限制

不兼容环境（非 4K 页）不支持直接运行 `ubse_ut`、各独立 UT 二进制（`ubse_rmrs_plugin_ut` 等）等测试二进制，请通过 `bash build.sh ut` 执行 UT。直接运行 `ctest` 亦不受该守卫保护，非 4K 页环境请勿使用。

## 6 单元测试报告生成

> [!IMPORTANT] 须知
>
> 只有当所有单元测试 **100% 通过**时，才允许生成覆盖率报告（CI 流水线强制校验）。

覆盖率报告在cmake-build-debug/coverage 目录下生成。
