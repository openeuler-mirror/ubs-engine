...
{
    return InvokeSdk([&]() { return ubs_npu_device_list_query(deviceList); });
}
...
{
    return InvokeSdk([&]() { return ubs_npu_device_alloc(allocInfo, newBusInstanceGuid, deviceList); });
}

int32_t ItSdkClient::NpuDeviceFree(ubs_ub_alloc_devices_info_t* allocInfo)
{
    return InvokeSdk([&]() { return ubs_npu_device_free(allocInfo); });
}
