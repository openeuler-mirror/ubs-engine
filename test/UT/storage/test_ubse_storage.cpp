...
TEST_F(TestUbseStorage, UbseStorageQueryData)
{
    ...
    MOCKER(&UbseStorageModule::Get)
            .stubs()
            .will(returnObjectList(UBSE_ERROR, UBSE_OK));
    EXPECT_EQ(UBSE_ERROR_NULLPTR, UbseStorageQueryData("key", "_ssu", data, nullptr));
    EXPECT_EQ(UBSE_ERROR_MODULE_LOAD_FAILED, UbseStorageQueryData("key", "_ssu", data, callbackFunc));
    EXPECT_EQ(UBSE_ERROR, UbseStorageQueryData("key", "_ssu", data, callbackFunc));
    EXPECT_EQ(UBSE_OK, UbseStorageQueryData("key", "_ssu", data, callbackFunc));
    module.reset();
}
