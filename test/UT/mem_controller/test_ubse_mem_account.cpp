 ...
 * 测试步骤：1.MOCKER UbseGetMemDebtInfoFromMaster，构造同一个节点同一个numa两次借出SHARE内存;
 * 2.调用UbseAllNumaInfo;
 ...
 * 测试步骤：1.MOCKER UbseGetMemDebtInfoFromMaster，构造同一个节点同一个numa两次借出numa内存;
 * 2.调用UbseAllNumaInfo;
