...
{
    // 测试获取节点信息失败
    MOCKER(ubse::mem::decoder::utils::MemDecoderUtils::GetCurNodeSocketInfo).stubs().will(returnValue(UBSE_ERROR));
    EXPECT_EQ(CycleCheckDecoderHandle(), UBSE_ERROR);

    // 测试获取所有句柄失败
    MOCKER(ubse::mem::decoder::utils::MemDecoderUtils::GetCurNodeSocketInfo).reset();
    MOCKER(ubse::mem::decoder::utils::MemDecoderUtils::GetCurNodeSocketInfo).stubs().will(returnValue(UBSE_OK));
    MOCKER(ubse::mem::decoder::utils::MemDecoderUtils::GetAllHandles).stubs().will(returnValue(UBSE_ERROR));
    EXPECT_EQ(CycleCheckDecoderHandle(), UBSE_ERROR);

    // 测试获取所有句柄成功，但获取导入对象失败
    MOCKER(ubse::mem::decoder::utils::MemDecoderUtils::GetAllHandles).reset();
    MOCKER(ubse::mem::decoder::utils::MemDecoderUtils::GetAllHandles).stubs().will(returnValue(UBSE_OK));
    MOCKER(ubse::mem::decoder::utils::MemDecoderUtils::GetAllHandleFromImportObj).stubs().will(returnValue(UBSE_ERROR));
    EXPECT_EQ(CycleCheckDecoderHandle(), UBSE_ERROR);

    // 测试删除句柄失败
    MOCKER(ubse::mem::decoder::utils::MemDecoderUtils::GetAllHandleFromImportObj).reset();
    MOCKER(ubse::mem::decoder::utils::MemDecoderUtils::GetAllHandleFromImportObj).stubs().will(returnValue(UBSE_OK));
    // Mock DelHandleByMapDiff 函数，使其返回非空的 faultInfo
    MOCKER(DelHandleByMapDiff).stubs().will(invoke(MockDelHandleByMapDiffFaultInfoNotEmpty));
    EXPECT_EQ(CycleCheckDecoderHandle(), UBSE_ERROR);

    // 测试所有操作成功
    MOCKER(DelHandleByMapDiff).reset();
    MOCKER(DelHandleByMapDiff).stubs().will(invoke(MockDelHandleByMapDiffFaultInfoEmpty));
...
TEST_F(TestUbseMemControllerModule, GetMemDebtInfo)
{
    ...
    MOCKER(UbseGetMemDebtInfoFromMaster).stubs().will(returnValue(UBSE_ERROR)).then(returnValue(UBSE_OK));
