...
TestUbseMemController::TestUbseMemController()
{
    memService_ = std::make_shared<UbseMemServiceImpl>();
    service::UbseServiceRegistry::GetInstance().RegisterService<UbseMemService>(memService_);
}
TestUbseMemController::~TestUbseMemController()
{
    service::UbseServiceRegistry::GetInstance().UnRegisterService<UbseMemService>(memService_);
}
...
TEST_F(TestUbseMemController, UbseGetNumaMemDebtInfoWithNode)
{
    ...
    MOCKER_CPP(&UbseGetMemDebtInfoFromMaster, uint32_t(const std::string &, NodeMemDebtInfoMap &))
        .stubs()
...
TEST_F(TestUbseMemController, UbseGetNumaMemDebtInfo)
{
    ...
    // 场景 1: 获取账本信息失败
    MOCKER(UbseGetMemDebtInfoFromMaster).stubs().will(returnValue(UBSE_ERROR));
    EXPECT_EQ(UbseGetNumaMemDebtInfo(debtInfos), UBSE_ERR_INTERNAL);

    // 场景 2: 获取账本信息成功
    MOCKER(UbseGetMemDebtInfoFromMaster).reset();
    MOCKER(UbseGetMemDebtInfoFromMaster).stubs().will(returnValue(UBSE_OK));
