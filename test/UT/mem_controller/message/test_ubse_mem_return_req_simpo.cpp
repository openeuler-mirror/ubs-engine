...
#include "test_ubse_mem_return_req_simpo.h"
...
void TestUbseMemReturnReqSimpo::SetUp()
{
    Test::SetUp();
    obj = new UbseMemReturnReqSimpo();
}

void TestUbseMemReturnReqSimpo::TearDown()
{
    Test::TearDown();
 ...
 */
TEST_F(TestUbseMemReturnReqSimpo, Serialize)
{
 ...
 * 2.给mInputRawData赋值，模拟并判断 UbseMemReturnReqDeserialize 在失败情况下Deserialize的返回值
 * 3.判断Check函数返回false和true时Deserialize的行为
 * 预期结果：
 * 1.初始状态函数返回UBSE_ERROR
 * 2. UbseMemReturnReqDeserialize 失败时函数返回UBSE_ERROR
 * 3.Check函数返回false时，Deserialize返回UBSE_ERROR
 * 4.一切功能正常时，返回UBSE_OK
 */
TEST_F(TestUbseMemReturnReqSimpo, Deserialize)
{
    ...
    MOCKER_CPP(&UbseMemReturnReqDeserialize).stubs().will(returnValue(false)).then(returnValue(true));
    EXPECT_TRUE(UBSE_ERROR == obj->Deserialize());
    EXPECT_TRUE(UBSE_OK == obj->Deserialize());

    MOCKER_CPP(&UbseMemReturnReqDeserialize).reset();
