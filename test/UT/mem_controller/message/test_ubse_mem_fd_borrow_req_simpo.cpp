...
void TestUbseMemFdBorrowReqSimpo::SetUp()
{
    Test::SetUp();
    obj = new UbseMemFdBorrowReqSimpo();
}

void TestUbseMemFdBorrowReqSimpo::TearDown()
{
    Test::TearDown();
 ...
 * 2.给mInputRawData赋值，模拟并判断UbseMemFdBorrowReqDeserialize在失败情况下Deserialize的返回值
 * 3.判断Check函数返回false和true时Deserialize的行为
 * 预期结果：
 * 1.初始状态函数返回UBSE_ERROR
 * 2.UbseMemFdBorrowReqDeserialize失败时函数返回UBSE_ERROR
 * 3.Check函数返回false时，Deserialize返回UBSE_ERROR
 * 4.一切功能正常时，返回UBSE_OK
 */
TEST_F(TestUbseMemFdBorrowReqSimpo, Deserialize)
{
