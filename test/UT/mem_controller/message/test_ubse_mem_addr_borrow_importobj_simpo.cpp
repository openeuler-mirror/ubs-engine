...
#include "test_ubse_mem_addr_borrow_importobj_simpo.h"
...
void TestUbseMemAddrBorrowImportobjSimpo::SetUp()
{
    GTEST_SKIP();
    Test::SetUp();
    obj = new UbseMemAddrBorrowImportobjSimpo();
}

void TestUbseMemAddrBorrowImportobjSimpo::TearDown()
{
    Test::TearDown();
 ...
 */
TEST_F(TestUbseMemAddrBorrowImportobjSimpo, Serialize)
{
 ...
 * 2.给mInputRawData赋值，模拟并判断UbseMemAddrBorrowImportObjDeserialization在失败和成功两种情况下Deserialize的返回值
 * 预期结果：
 * 1.初始状态函数返回UBSE_ERROR
 * 2.UbseMemAddrBorrowImportObjDeserialization失败时函数返回UBSE_ERROR
 * 3.UbseMemAddrBorrowImportObjDeserialization成功时函数返回UBSE_OK
 */
TEST_F(TestUbseMemAddrBorrowImportobjSimpo, Deserialize)
{
    ...
    MOCKER_CPP(UbseMemAddrBorrowImportObjDeserialization)
        .stubs()
