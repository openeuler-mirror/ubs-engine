...
#include "test_ubse_mem_fd_borrow_exportobj_simpo.h"
...
void TestUbseMemFdBorrowExportobjSimpo::SetUp()
{
    Test::SetUp();
    obj = new UbseMemFdBorrowExportobjSimpo();
}

void TestUbseMemFdBorrowExportobjSimpo::TearDown()
{
    Test::TearDown();
 ...
 */
TEST_F(TestUbseMemFdBorrowExportobjSimpo, Serialize)
{
 ...
 * 2.给mInputRawData赋值，模拟并判断UbseMemFdBorrowExportObjDeserialization在失败和成功两种情况下Deserialize的返回值
 * 预期结果：
 * 1.初始状态函数返回UBSE_ERROR
 * 2.UbseMemFdBorrowExportObjDeserialization失败时函数返回UBSE_ERROR
 * 3.UbseMemFdBorrowExportObjDeserialization成功时函数返回UBSE_OK
 */
TEST_F(TestUbseMemFdBorrowExportobjSimpo, Deserialize)
{
