...
void TestUbseMemAddrBorrowExportobjSimpo::SetUp()
{
    GTEST_SKIP();
    Test::SetUp();
    obj = new UbseMemAddrBorrowExportobjSimpo();
}
 ...
 * 2.给mInputRawData赋值，模拟并判断 UbseMemAddrBorrowExportObjDeserialization 在失败和成功两种情况下Deserialize的返回值
 * 预期结果：
 * 1.初始状态函数返回UBSE_ERROR
 * 2. UbseMemAddrBorrowExportObjDeserialization 失败时函数返回UBSE_ERROR
 * 3. UbseMemAddrBorrowExportObjDeserialization 成功时函数返回UBSE_OK
 */
TEST_F(TestUbseMemAddrBorrowExportobjSimpo, Deserialize)
{
    ...
    MOCKER_CPP(&UbseMemAddrBorrowExportObjDeserialization)
        .stubs()
