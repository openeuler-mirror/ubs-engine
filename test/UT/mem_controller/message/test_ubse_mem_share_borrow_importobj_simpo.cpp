...
void TestUbseMemShareBorrowImportobjSimpo::SetUp()
{
    GTEST_SKIP();
    Test::SetUp();
    obj = new UbseMemShareBorrowImportobjSimpo();
}
 ...
 * 2.给mInputRawData赋值，模拟并判断 UbseMemShareBorrowImportObjDeserialization 在失败和成功两种情况下Deserialize的返回值
 * 预期结果：
 * 1.初始状态函数返回UBSE_ERROR
 * 2. UbseMemShareBorrowImportObjDeserialization 失败时函数返回UBSE_ERROR
 * 3. UbseMemShareBorrowImportObjDeserialization 成功时函数返回UBSE_OK
 */
TEST_F(TestUbseMemShareBorrowImportobjSimpo, Deserialize)
{
    ...
    MOCKER_CPP(&UbseMemShareBorrowImportObjDeserialization)
        .stubs()
