...
TEST_F(TestUbseMemDebtInfoQueryAddr, AddrGet_ImportResultsEmpty)
{
    ...
    numaInfo.size = 4096;
    obj->algoResult.exportNumaInfos.push_back(numaInfo);
    UbseMemDebtLedger::GetInstance().GetDebtMap<UbseMemAddrBorrowImportObj>().PutResource("importNode", "test", obj);
...
TEST_F(TestUbseMemDebtInfoQueryAddr, AddrGet_Success)
{
    ...
    UbseMemDebtLedger::GetInstance().GetDebtMap<UbseMemAddrBorrowImportObj>().PutResource("importNode", "test",
                                                                                          importObj);
