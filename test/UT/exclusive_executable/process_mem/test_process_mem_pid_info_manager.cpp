...
TEST_F(TestProcessMemPidInfoManager, RemovePidReBorrowThenRemoveNameWithMockPids)
{
    auto& mgr = ProcessMemPidInfoManager::GetInstance();
    InjectMockPidLifecycleState(MockPidLifecycleKeys());
    mgr.RefreshProcMemConfigCache();
    ...
    freedDebts.clear();
    InjectMockPidLifecycleState({MOCK_NAME, std::to_string(MOCK_PID_C)});
    EXPECT_EQ(mgr.RemoveProcMemConfig(true, std::to_string(MOCK_PID_B)), UBSE_OK);
    ASSERT_EQ(freedDebts.size(), 1u);
    EXPECT_EQ(freedDebts[0], "debt-B");
    EXPECT_EQ(mgr.GetProcMemConfig(true, std::to_string(MOCK_PID_B)).maxMemory, 0u);
    snapshot = mgr.GetManagedPidCacheSnapshot();
    EXPECT_EQ(snapshot.find(MOCK_PID_B), snapshot.end());
    ASSERT_NE(snapshot.find(MOCK_PID_C), snapshot.end());

    InjectMockPidLifecycleState({std::to_string(MOCK_PID_B), MOCK_NAME, std::to_string(MOCK_PID_C)});
    mgr.RebuildManagedPidCache();
    snapshot = mgr.GetManagedPidCacheSnapshot();
    ASSERT_NE(snapshot.find(MOCK_PID_B), snapshot.end());
    EXPECT_TRUE(snapshot.at(MOCK_PID_B).sources & static_cast<uint8_t>(ConfigSource::NAME_CONFIG));

    auto& decision = ProcessMemPidDecision::GetInstance();
    if (decision.borrowExecutor_ == nullptr || !decision.borrowExecutor_->IsStart()) {
        decision.borrowExecutor_ = ubse::task_executor::UbseTaskExecutor::Create("ProcMemInfoMgrLifecycle", 2, 64);
    ...
    decision.freeMemoryThresholdBytes_ = 50 * 1073741824ull;
    decision.emergencyThresholdBytes_ = 5 * 1073741824ull;
    constexpr uint64_t need = 1 * 1073741824ull;
    ubse::mem::controller::MockSetNumaCreateDesc(3, 7, need);
    collect::PidCollectInfoMap collectMap;
    collectMap.entries[MOCK_PID_B].vmRssKb = 2 * 1024 * 1024;
    mgr.UpdateManagedPidVmRssBatch(collectMap);
    ubse::mem::controller::UbseNodeNumaInfo nodeInfo{};
    nodeInfo.nodeId = "NODE0";
    nodeInfo.numaId = 0;
    nodeInfo.memFree = 45 * 1073741824ull;
    ubse::mem::controller::MockSetNodeNumaInfos({nodeInfo});
    decision.OnDecisionTimer();

    snapshot = mgr.GetManagedPidCacheSnapshot();
    ASSERT_EQ(snapshot.at(MOCK_PID_B).borrow.slots.size(), 1u);
    EXPECT_EQ(snapshot.at(MOCK_PID_B).borrow.slots[0].status, BorrowSlotStatus::COMPLETED);
    EXPECT_EQ(snapshot.at(MOCK_PID_B).borrow.slots[0].capacity, need);
    EXPECT_EQ(snapshot.at(MOCK_PID_B).borrow.currentRemote, need);

    auto newDebtBName = ubse::mem::controller::MockGetLastNumaCreateName();
    ASSERT_FALSE(newDebtBName.empty());
    ubse::mem::controller::MockSetDebtInfos(
        {MakeDebtInfo(newDebtBName, MOCK_PID_B), MakeDebtInfo("debt-C", MOCK_PID_C)});

    freedDebts.clear();
    EXPECT_EQ(mgr.RemoveProcMemConfig(false, MOCK_NAME), UBSE_OK);
    ASSERT_EQ(freedDebts.size(), 2u);
    EXPECT_EQ(freedDebts[0], newDebtBName);
    EXPECT_EQ(freedDebts[1], "debt-C");
    EXPECT_EQ(mgr.GetProcMemConfig(false, MOCK_NAME).maxMemory, 0u);
    InjectMockPidLifecycleState({});
    mgr.RebuildManagedPidCache();
    snapshot = mgr.GetManagedPidCacheSnapshot();
    EXPECT_TRUE(snapshot.empty());
