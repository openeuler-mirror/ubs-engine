...
{
    std::map<std::string, NodeMemoryInfo> MyNodes = {
        {"A", {
            .usedMemory = 819200,
            .freeMemory = 204800,
...
TEST_F(MigrateStrategyTest, MemoryMigrationStrategy_BasicScenario)
{
    ...
    std::map<std::string, std::map<std::string, PageCacheSensitiveTag>> nodeTags = {
        {"A", {
            {"DOCKER1", PageCacheSensitiveTag::SENSITIVE},
            {"DOCKER2", PageCacheSensitiveTag::NOT_SENSITIVE}
        }},
        {"B", {
            {"DOCKER3", PageCacheSensitiveTag::SENSITIVE}
        }},
        {"C", {
            {"DOCKER4", PageCacheSensitiveTag::SENSITIVE},
            {"DOCKER5", PageCacheSensitiveTag::NOT_SENSITIVE}
        }},
        {"D", {
            {"DOCKER6", PageCacheSensitiveTag::SENSITIVE},
            {"DOCKER7", PageCacheSensitiveTag::NOT_SENSITIVE}
        }}
    };

    std::vector<MigrationAction> expectedActions = {
        MigrationAction{
            .fromNode = "A",
            .dockerIds = {"DOCKER1"},
            .toNode = "C",
