...
TEST_F(TestUbseUrmaControllerApi, UbseUrmaBandWidthSet_NameTooLong)
{
    std::string longName(33, 'x');
    uint8_t *buffer = new uint8_t[longName.size() + 1];
    memcpy(buffer, longName.c_str(), longName.size() + 1);
    UbseIpcMessage req = {buffer, longName.size() + 1};
    UbseRequestContext ctx = {};
    auto ret = UbseUrmaControllerApi::UbseUrmaBandWidthSet(req, ctx);
    EXPECT_EQ(ret, UBSE_ERROR);
    delete[] buffer;
}

TEST_F(TestUbseUrmaControllerApi, UbseUrmaBandWidthGet_NullBuffer)
{
    UbseIpcMessage req = {nullptr, 0};
    UbseRequestContext ctx = {};
    auto ret = UbseUrmaControllerApi::UbseUrmaBandWidthGet(req, ctx);
    EXPECT_EQ(ret, UBSE_ERROR_NULLPTR);
}

TEST_F(TestUbseUrmaControllerApi, UbseUrmaBandWidthReset_NameTooLong)
{
    std::string longName(33, 'x');
    uint8_t *buffer = new uint8_t[longName.size() + 1];
    memcpy(buffer, longName.c_str(), longName.size() + 1);
    UbseIpcMessage req = {buffer, longName.size() + 1};
    UbseRequestContext ctx = {};
    auto ret = UbseUrmaControllerApi::UbseUrmaBandWidthReset(req, ctx);
    EXPECT_EQ(ret, UBSE_ERROR);
    delete[] buffer;
}

TEST_F(TestUbseUrmaControllerApi, UbseUrmaBandWidthCliGet_DeserializeFail)
{
    uint8_t badData[4] = {0};
    UbseIpcMessage req = {badData, sizeof(badData)};
    UbseRequestContext ctx = {};
    auto ret = UbseUrmaControllerApi::UbseUrmaBandWidthCliGet(req, ctx);
    EXPECT_NE(ret, 0);
}

TEST_F(TestUbseUrmaControllerApi, UbseUrmaSendQosRsp_NullApiModule)
{
    std::shared_ptr<UbseApiServerModule> nullMod;
    MOCKER_CPP(&UbseContext::GetModule<UbseApiServerModule>).stubs().will(returnValue(nullMod));
    auto ret = UbseUrmaControllerApi::UbseUrmaSendQosRsp(0, 100, 200);
    EXPECT_EQ(ret, UBSE_ERROR_NULLPTR);
}
