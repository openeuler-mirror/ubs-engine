    ...
    std::vector<mempooling::outinterface::VmDomainInfo> &outVmDomainInfos)
{
    ...
    try {
        ...
        UBSE_LOGGER_ERROR(MP_MODULE_NAME, MP_MODULE_CODE) << "UBSRMRSGetVmInfoListOnNode caught std::exception: " << e.what();
        return MEM_POOLING_ERROR;
    } catch (...) {
                                                            ...
                                                            const std::optional<std::vector<uint32_t>> &durationVec)
{
    ...
    int ret =
        smap::MpSmapHelper::SmapAddProcessTrackingHelper(pidVec, scanTimeVec, scanType, durationVecParam);
    if (ret == SMAP_OK) {
...
int mempooling::outinterface::RemoteNumaMigrate(const std::vector<pid_t>& pids, int srcNid, int destNid)
{
    ...
    if (smapMigratePidRemoteNumaFunc == nullptr) {
        UBSE_LOGGER_ERROR(MP_MODULE_NAME, MP_MODULE_CODE)
            << "Ptr smapMigratePidRemoteNumaFunc == nullptr.";
        return MEM_POOLING_ERROR;
    }
...
int mempooling::outinterface::MigrateOut(const std::vector<MigrateOutPayload> &items, int pidType)
{
    ...
    if (ret == 0) {
        ...
        UBSE_LOGGER_ERROR(MP_MODULE_NAME, MP_MODULE_CODE)
            << "MigrateOut SmapMode failed, error_code=" << ret << ".";
        return ret;
    }
