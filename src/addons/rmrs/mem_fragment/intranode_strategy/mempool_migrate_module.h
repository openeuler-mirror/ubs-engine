...
public:
    ...
    static bool FillDestNumaFreeHugePageMap(
    ...
    // 回滚归还前判定该借据是否需要smap迁回：仅当借据关联的pid仍被smap纳管
    // 且在借用的远端numa上有实际占用时才需要迁回，否则直接归还；
    // 查询失败时保守返回true，维持原smap迁回路径，避免带数据误走直接归还
    ...
    static MpResult BatchBorrowStrategyImpl(const std::string& srcNid, const std::vector<int16_t>& srcNumaIds,
                                            uint64_t borrowSize,
...
public:
    ...
    static MpResult MemBorrowRollbackImpl(std::vector<std::string>& borrowIds, const RollBackBorrowIdPid& entryList,
