...
public:
    ...
    static std::vector<mempooling::VMPresetParam> ConvertToVmPresetParam(const std::vector<pid_t>& pids, int16_t numaId,
                                                                               uint64_t curBorrowMemorySize = 0);
...
private:
    ...
    static float CalculateMemMigrateRatio(int16_t numaId, uint64_t curBorrowMemorySize);
};
} // namespace vm
