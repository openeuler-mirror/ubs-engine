                                          ...
                                          const std::vector<uint64_t>& borrowSizes)
{
    ...
    VMNodeLocInfo vmNodeLocInfo = {
        .hostId = originNode.hostId,
    ...
    const vector<VMPresetParam>& vmPresetParam = ConvertToVmPresetParam(pids, originNode.numaId);
...
void StatusManager::CleanEmptyBorrowRes(MemBorrowExecuteResult& result)
{
    ...
    // Iterate through borrowIds and presentNumaIds. If a borrowId is not bb empty, add it to newBorrowIds and its
...
float StatusManager::CalculateMemMigrateRatio(int16_t numaId)
{
    double totalBorrowedMem = 0;
    ...
    for (const auto& [nodeLoc, vmMap] : globalNumaVmInfoMap) {
        ...
        for (const auto& [uuid, vmInfo] : vmMap) {
            ...
            totalVMusedMem += static_cast<double>(vmInfo.remoteUsedMem / NO_1024);
        }
    }
    totalBorrowedMem /= NO_1024; // Bytes to KB
    UBSE_LOG_DEBUG << "totalBorrowedMem = " << std::to_string(totalBorrowedMem)
                   << ", totalVMusedMem = " << std::to_string(totalVMusedMem);
    float ratio = static_cast<float>((totalBorrowedMem * 100) / totalVMusedMem);
    return ratio;
}
...
std::vector<VMPresetParam> StatusManager::ConvertToVmPresetParam(const std::vector<pid_t>& pids, int16_t numaId)
{
    ...
    float ratio = CalculateMemMigrateRatio(numaId);
