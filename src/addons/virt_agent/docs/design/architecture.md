...
UBS VirtAgent是UB场景下提供虚拟化（或容器化）特性的核心组件，仅适用于通用计算场景；其包含虚拟化、容器化两大核心场景。具体提供一下特性：
...
在部署形态上，虚拟化与容器化场景大同小异。在说明部署形态之前，需要引入MatrxiServer的概念，MatrixServer是构建虚拟化特性的最小部署单元，其中包含云管生态的插件服务、提供UB生态特性并封装高级特性的VirtAgent组件、以及封装UB生态特性的UBS Engine框架。一个云管集群，一般可以对接一个或多个MatrxiServer集群。
