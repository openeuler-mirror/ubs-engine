...
其中，内存相关的借用、归还、冷热页流动等能力，依赖UBS Engine与UBS RMRS等UB生态中其他组件提供。UBS VirtAgent组件，定位是直接与云管生态交互的中间件，让云管生态能轻松地使用UB生态特性，实现更多上述场景下的高级特性。
...
UBS VirtAgent依赖相对来说比较简单，由于是直接对接云管生态的服务。只有两个底层依赖：
