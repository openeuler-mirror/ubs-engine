...
| 特性    | virtagent提供的能力                                                      |
|-------|---------------------------------------------------------------------|
| 虚拟化超分逃生 | 虚拟化超分场景，内存使用率超过阈值，触发内存借用逃生                                          |
| 虚拟化内存碎片 | 虚拟化内存碎片场景，节点cpu充足、内存不足时，允许借用邻居节点内存，创建虚拟机                            |
| 大规格虚机 | 此为虚拟化内存碎片特殊场景，支持通过内存借用的方式，创建/删除大规格虚拟机（1~6T）                         |
| 容器化超分逃生 | 基于UB内存池化机制，裸机容器场景通过使能内存无感借用                                         |
| 确定性热迁移 | 虚拟化超分场景，内存使用率超过阈值，触发虚拟机热迁移逃生                                        |
| 异地快恢  | OS Panic故障快恢场景，VirtAgent提供内存借用能力，用于快恢VM发放前借出正常工作VM的内存到容灾VM；包括内存释放接口 |
