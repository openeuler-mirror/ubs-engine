...
public:
     ...
     */
    uint32_t SetNumaNodeBorrowSize(
        const std::string &memName, const std::string &from, const std::string &to, int fromNumaId, int toNumaId);
     ...
     * @fromNumaId：入参，借出内存在借出节点的numaId
     */
    uint32_t DelNumaNodeBorrowSize(
        const std::string &memName, const std::string &from, const std::string &to, int fromNumaId);
