...
- **阈值配置关系**：`emergency_free_threshold` 应小于 `pressing_free_threshold`，且 `pressing_free_threshold` 应小于本地总内存。否则可能出现借用持续无法触发，或空闲内存回升后永远达不到主动归还条件的情况。触发水位与系统 OOM 底线之差即模型中的水位余量，决定了节点能承受的内存上涨速率，具体估算见第 6 章。
...
纳管进程内存上涨是本地空闲内存跌破水位的直接原因，上涨过快时空闲内存会被击穿直至触发 OOM。本章给出容量整定用的参考模型：6.1 先定义模型口径，6.2 给出可承受上涨速率的计算与反推。模型用于估算阈值该怎么配，不是插件内部的强约束。

## 6.1 模型口径

| 名称 | 含义 | 单位 |
| ---- | ---- | ---- |
| 上涨速率 | 纳管进程内存增长的速率，等效为本地空闲内存被消耗的速率 | GB/s |
| 上涨持续时长 | 一轮内存上涨从开始到结束的持续时间 | 秒 |
| 反应窗口 | 从内存开始上涨到第一次借用把页面迁出为止的延迟 = 快照陈旧时长（即采集周期）+ 决策周期 + 首次扫描耗时（首次触发借用的那一轮内、对每个候选进程串行读取 `/proc/<pid>/numa_maps` 的耗时，随纳管进程数与映射规模增长） | 秒 |
| 触发水位 | 配置项 `pressing_free_threshold`，本地空闲内存低于该值触发借用 | GB |
| OOM 最低水位 | 系统触发 OOM 的内存底线，节点空闲内存跌到该底线附近即可能触发系统 OOM；由用户按部署环境自行评估，process_mem 无对应配置项，与 `emergency_free_threshold`（紧急借用触发线）不是同一个量 | GB |
| 水位余量 | 触发水位 − OOM 最低水位，不触发 OOM 的前提下可被上涨消耗的余量 | GB |
| 标称迁移带宽 | SMAP 给出的页面迁移速率规格 | GB/s |
| 迁移带宽损失比 | 迁移与业务争抢、链路损耗等因素导致的带宽折损比例 | - |
| 有效迁移带宽 | 标称迁移带宽 ×（1 − 损失比） | GB/s |
| 窗口期降幅 | 反应窗口内没有迁移进账，由水位余量全额承担的消耗量 | GB |
| 全程降幅 | 整段上涨（含反应窗口）期间水位余量被消耗的总量，即上涨结束时水位相对触发水位的降幅 | GB |

> OOM 最低水位取决于系统侧（系统预留、内核保留等），需部署时自行评估。

> 反应窗口前两项由配置决定（`collect_process_interval` 与 `schedule_interval` 默认各 5 秒）；第三项扫描时间实测 3~5 秒，按最大 5 秒计入。

整定用的迁移带宽取值：

| 项 | 取值 | 来源 |
| ---- | ---- | ---- |
| 标称迁移带宽 | 1GB/s | SMAP 规格值 |
| 迁移带宽损失比 | **55%** | 实测平均值 |
| 有效迁移带宽 | **约 0.45GB/s** | 标称迁移带宽 ×（1 − 损失比），整定按该值估算 |

> 现场条件不同（业务争抢程度、链路差异）时损失比会有出入，必要时按实测值替换。

## 6.2 模型

可承受的上涨速率由窗口期降幅与全程降幅两个约束决定，两者取严：

- 窗口期约束：`窗口期降幅 = 上涨速率 × min(反应窗口, 上涨持续时长) ≤ 水位余量`
- 全程约束：`全程降幅 = 上涨速率 × 上涨持续时长 − 有效迁移带宽 × max(0, 上涨持续时长 − 反应窗口) ≤ 水位余量`

由此得到最大可承受上涨速率：

```
可承受上涨速率 = min( 水位余量 ÷ min(反应窗口, 上涨持续时长),
                      [水位余量 + 有效迁移带宽 × max(0, 上涨持续时长 − 反应窗口)] ÷ 上涨持续时长 )
```

反推形式（按要扛住的上涨速率求触发水位，用于容量规划）：

```
所需触发水位 = 上涨速率 × min(反应窗口, 上涨持续时长)
             + max(0, 上涨速率 − 有效迁移带宽) × max(0, 上涨持续时长 − 反应窗口)
             + OOM 最低水位
```

> 模型边界：上涨速率按恒速线性上涨计；只计入纳管进程的上涨，非纳管进程的消耗不在模型内；假定迁出能按有效迁移带宽持续到上涨结束，未考虑单进程可迁移上限（内存占用 × `remoteRatio`）与借出节点无空闲时的被动归还；水位余量按上涨恰好发生在触发水位附近取值，属保守口径。

# 7 内存开销估算与 MemoryMax 配置
