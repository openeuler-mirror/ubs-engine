...
#include "ubse_mem_share_borrow_importobj_simpo.h"
...
UbseResult UbseMemShareBorrowImportobjSimpo::Serialize()
{
    UbseSerialization out;
    if (!UbseMemShareBorrowImportObjSerialization(out, importObj_)) {
...
UbseResult UbseMemShareBorrowImportobjSimpo::Deserialize()
{
    ...
    if (!UbseMemShareBorrowImportObjDeserialization(in, importObj_)) {
