...
#include "ubse_mem_addr_borrow_exportobj_simpo.h"
...
UbseResult UbseMemAddrBorrowExportobjSimpo::Serialize()
{
    UbseSerialization out;
    if (!UbseMemAddrBorrowExportObjSerialization(out, exportObj_)) {
...
UbseResult UbseMemAddrBorrowExportobjSimpo::Deserialize()
{
    ...
    if (!UbseMemAddrBorrowExportObjDeserialization(in, exportObj_)) {
