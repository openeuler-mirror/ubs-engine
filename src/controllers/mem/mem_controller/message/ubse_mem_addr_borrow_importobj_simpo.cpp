...
#include "ubse_mem_addr_borrow_importobj_simpo.h"
...
UbseResult UbseMemAddrBorrowImportobjSimpo::Serialize()
{
    UbseSerialization out;
    if (!UbseMemAddrBorrowImportObjSerialization(out, importObj_)) {
...
UbseResult UbseMemAddrBorrowImportobjSimpo::Deserialize()
{
    ...
    if (!UbseMemAddrBorrowImportObjDeserialization(in, importObj_)) {
