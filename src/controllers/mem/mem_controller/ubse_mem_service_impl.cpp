                          ...
                          std::unordered_map<std::string, UbseNumaMemoryDebtInfo> &numaMemoryDebtInfoMap)
{
    ...
    // 处理用户私有数据
    if (memcpy_s(debtInfo.usrInfo, controller::UBSE_MAX_USR_INFO_LEN, numaImportObj.req.usrInfo,
                 controller::UBSE_MAX_USR_INFO_LEN) != UBSE_OK) {
                          ...
                          std::unordered_map<std::string, UbseNumaMemoryDebtInfo> &numaMemoryDebtInfoMap)
{
    ...
    // 处理用户私有数据
    if (memcpy_s(debtInfo.usrInfo, controller::UBSE_MAX_USR_INFO_LEN, numaExportObj.req.usrInfo,
                 controller::UBSE_MAX_USR_INFO_LEN) != UBSE_OK) {
                          ...
                          std::unordered_map<std::string, UbseMemAddrDesc> &addrMemoryDebtInfoMap)
{
    ...
    if (!addrBorrowImportObj.status.importResults.empty()) {
...
uint32_t UbseMemServiceImpl::UbseQueryResult(const std::string &name, UbseMemResult &result,
...
UbseResult UbseMemServiceImpl::UbseGetNumaMemDebtInfoWithNode(const std::string &nodeId,
                                                              std::vector<UbseNumaMemoryDebtInfo> &debtInfos)
{
    ...
    uint32_t ret = UbseGetMemDebtInfoFromMaster("", memDebtInfoMap);
    if (ret != UBSE_OK) {
...
UbseResult UbseMemServiceImpl::UbseGetNumaMemDebtInfo(std::vector<UbseNumaMemoryDebtInfo> &debtInfos)
{
    // 获取节点信息
    std::set<uint32_t> staticNodeInfoList = UbseNodeController::GetInstance().UbseGetAllDeployedNode();
    std::unordered_map<std::string, ubse::nodeController::UbseNodeInfo> nodeMap =
...
UbseResult UbseMemServiceImpl::UbseGetNumaMemImportDebtInfoWithLocalNode(
...
UbseResult UbseMemServiceImpl::UbseGetAllNodeNumaInfo(std::vector<UbseNodeNumaInfo> &numaNodeInfoList)
{
...
UbseResult UbseMemServiceImpl::UbseGetNodeNumaInfoByNodeId(const std::string &nodeId,
...
UbseResult UbseMemServiceImpl::UbseMemNumaCreate(const std::string &name, const UbseMemBorrower &borrower,
                                                 const UbseMemNumaCreateOpt &opt, UbseMemNumaDesc &desc)
{
    ...
    int32_t retCode = UbseMemNumaGetWithImportNode(resp.name, borrower.nodeId, desc);
    if (retCode != UBSE_OK) {
        UBSE_LOG_ERROR << "UbseMemNumaGet, ErrorCode=" << retCode;
    }
    return resp.errorCode;
...
UbseResult UbseMemServiceImpl::UbseMemNumaCreateWithCandidate(const std::string &name, const UbseMemBorrower &borrower,
...
UbseResult UbseMemServiceImpl::UbseMemNumaDelete(const std::string &name, const UbseMemBorrower &borrower)
{
    // 参数校验
                                                 ...
                                                 UbseMemAddrDesc &desc)
{
    ...
    int32_t retCode = ubse::mem::controller::UbseMemAddrGet(resp.name, opt.borrower.nodeId, desc);
    if (retCode != UBSE_OK) {
...
UbseResult UbseMemServiceImpl::UbseMemAddrDelete(const std::string &name, const UbseMemBorrower &borrower)
{
    // 参数校验
    auto ret = UbseMemDeleteReqIsValid(name, borrower);
    if (ret != UBSE_OK) {
...
UbseResult UbseMemServiceImpl::UbseMemDebtCircleCheck(const std::string &srcNodeId, const std::string &dstNodeId,
                                                      bool &isCircle)
{
...
UbseResult UbseMemServiceImpl::UbseGetAddrMemDebtInfoWithNode(const std::string &nodeId,
                                                              std::vector<UbseMemAddrDesc> &debtInfos)
{
...
uint32_t UbseMemServiceImpl::UbseGetMemDebtInfoFromMaster(const std::string &nodeId, NodeMemDebtInfoMap &memDebtInfoMap)
{
    return controller::UbseGetMemDebtInfoFromMaster(nodeId, memDebtInfoMap);
}
NodeMemDebtInfoMap UbseMemServiceImpl::UbseGetLocalMemDebtInfo()
{
...
uint32_t UbseMemServiceImpl::PreImportDecoderEntry(const PreImportDecoderParam &importDecoderParam,
                                                   ubse::adapter_plugins::mti::mami::UbseMamiMemImportResult &outValue)
{
    return mem::decoder::utils::MemDecoderUtils::PreImportDecoderEntry(importDecoderParam, outValue);
}
bool UbseMemServiceImpl::IsNeedPreOnline(const DecoderEntryLoc &loc, uint32_t dcna,
                                         ubse::adapter_plugins::mti::mami::UbseMamiMemImportResult &outValue)
{
    return mem::decoder::utils::UbseMemPrehandleManager::GetInstance().IsNeedPreOnline(loc, dcna, outValue);
}
void UbseMemServiceImpl::RollbackPreImportHandle(const DecoderEntryLoc &loc)
{
    mem::decoder::utils::UbseMemPrehandleManager::GetInstance().RollbackPreImportHandle(loc);
}
UbseResult UbseMemServiceImpl::InitPreHandle(const std::vector<decoder::utils::BasicPreImportInfo> &preImportInfos)
{
    return mem::decoder::utils::UbseMemPrehandleManager::GetInstance().InitPreHandle(preImportInfos);
}
} // namespace ubse::service::mem