...
bool NodeControllerTryReadLock(const UbseMemAddrBorrowReq& req)
{
    ...
    if (!UbseNodeControllerLockMgr::TryReadLock(req.exportNodeId)) {
        ...
        UBSE_LOG_ERROR << "TryReadLock exportNode " << req.exportNodeId << " failed, requestId=" << req.requestId;
        return false;
    }
    return true;
}
...
uint32_t DoUbseMemAddrBorrow(const std::string& exportKey, const UbseMemAddrBorrowReq& req, UbseMemOperationResp& resp)
{
    ...
    if (auto ret = SendAddrExportObj(exportObj, true, req.exportNodeId); ret != UBSE_OK) {
        ...
        BorrowFailedAdvice({MemFault::BORROW_MASTER_TO_EX_SEND_FAILED, req.name, MemType::ADDR, GetAddrReqSize(req),
                            req.exportNodeId, req.importNodeId, req.requestNodeId});
                           ...
                           const std::string &realRequestNodeId)
{
    ...
    << ", importNodeId=" << req.importNodeId << ", exportAccessMode=" << req.exportAccessMode;    resp.requestId = req.requestId;
    if (!IsMemBorrowFeatureSupported()) {
        BorrowFailedAdvice({MemFault::BORROW_CHIP_NOT_SUPPORTED, req.name, MemType::ADDR, GetAddrReqSize(req),
                            req.exportNodeId, req.importNodeId, req.requestNodeId});
    ...
    if (ret != UBSE_OK) {
        BorrowFailedAdvice({MemFault::BORROW_FAULT_INTERNAL, req.name, MemType::ADDR, GetAddrReqSize(req),
                            req.exportNodeId, req.importNodeId, req.requestNodeId});
    ...
    if (errCode != UBSE_ERR_NOT_EXIST) {
        BorrowFailedAdvice({MemFault::BORROW_NAME_EXIST, req.name, MemType::ADDR, GetAddrReqSize(req), req.exportNodeId,
                                         ...
                                         const std::string& importNodeId, const std::string& name)
{
    ...
    if (exportObj.status.state == UBSE_MEM_EXPORT_SUCCESS) { // 导出成功 开始导入
        ...
        if (ret != UBSE_OK) {
            UBSE_LOG_ERROR << "Failed to get cna info when inport, requestId=" << exportObj.req.requestId
            << ", " << FormatRetCode(ret);
        ...
        if (ret = SendAddrImportObj(importObj, true, importNodeId); ret != UBSE_OK) {
            UBSE_LOG_ERROR << "Failed to send addr import. name=" << name << ", requestId=" << exportObj.req.requestId;
            BorrowFailedAdvice({MemFault::BORROW_MASTER_TO_IM_SEND_FAILED, name, MemType::ADDR,
                                GetAddrReqSize(importObj.req), exportNodeId, importNodeId, importNodeId});
            return AddrExportRollback(resp, exportObj, importObj, exportNodeId, name);
        }
        return UBSE_OK;
    }
    // 导出失败，删除导入/导出记录
    UBSE_LOG_INFO << "[MMC] import failed, start to rollback with deleting export and import, requestId="
    << exportObj.req.requestId;
                                               ...
                                               const std::string& name)
{
    ...
    if (exportObj.status.state == UBSE_MEM_EXPORT_SUCCESS) {
        ...
        if (auto ret = BuildOperationRespWhenFail(resp, name, requestNodeId, "Failed to unexport.",
						  UBSE_ERR_UNIMPORT_SUCCESS, MemOperationType::ADDR_RETURN); ret != UBSE_OK) {
            BorrowFailedAdvice({MemFault::RETURN_MASTER_TO_REQ_SEND_FAILED, name, MemType::ADDR, 0,
                                exportObj.req.exportNodeId, requestNodeId, requestNodeId});
            return ret;
        }
        return UBSE_OK;
    }
    ...
    EraseAddrExport(exportObj);
    UBSE_LOG_INFO << "[MMC] return success , requestId=" << exportObj.returnReq.requestId;
    SchedulerImpl::GetInstance().MemoryObjChangeHandler(exportObj); // 通知算法
    // requestNodeId为空则当前场景为对账删除导出账本或者借用失败回滚
    if (requestNodeId.empty()) {
        UBSE_LOG_INFO << "requestNodeId is empty , name=" << name << ", requestId=" << exportObj.returnReq.requestId;
        return UBSE_OK;
    }
    if (auto ret = BuildOperationRespWhenSuccess(resp, UBSE_OK, MemOperationType::ADDR_RETURN); ret != UBSE_OK) {
        BorrowFailedAdvice({MemFault::RETURN_MASTER_TO_REQ_SEND_FAILED, name, MemType::ADDR, 0,
                            exportObj.req.exportNodeId, requestNodeId, requestNodeId});
        return ret;
    }
                                           ...
                                           const std::string& exportNodeId)
{
    ...
    if (auto ret = UbseMmiInterface::GetInstance().AddrUnExportExecutor(exportObj); ret != UBSE_OK) {
        UBSE_LOG_ERROR << "Failed to unexport name=" << name << ", requestId=" << exportObj.returnReq.requestId;
        size_t reqSize = 0;
    ...
    // 归还成功
    UBSE_LOG_INFO << "Success to unexport addr, name=" << name << ", requestId=" << exportObj.returnReq.requestId;
                             ...
                             const std::string& name)
{
    ...
    importParam.type = decoder::utils::DecoderBorrowType::ADDR;
    {
        ...
        if (res != UBSE_OK) {
            UBSE_LOG_ERROR << "ImportToAddDecoderEntry failed, res=" << res << ", requestId=" << importObj.req.requestId;
    ...
    UBSE_LOG_INFO << "Success to import, name=" << name << ", requestId=" << importObj.req.requestId;
                                     ...
                                     const std::string& name)
{
    ...
    auto res = decoder::utils::MemDecoderUtils::GetChipAndDieId(importObj.algoResult.attachSocketId, chipDiePair);
    if (res != UBSE_OK) {
        UBSE_LOG_ERROR << "GetChipAndDieId by socketId failed, requestId=" << importObj.req.requestId;
    ...
    UBSE_LOG_INFO << "Success to unimport addr, name=" << name << ", requestId=" << importObj.req.requestId;
    ...
    if (!importObj.status.decoderResult.empty()) {
        UBSE_LOG_ERROR << "UnimportToDelDecoderEntry failed, requestId=" << importObj.req.requestId;
    }
    return UBSE_OK;
}
                                      ...
                                      const std::string& name)
{
    ...
    if (res != UBSE_OK) {
        ...
        UBSE_LOG_ERROR << "AddrUnImport Failed, Failed count:" << ++g_addrUnimportFailedCount << ". advice: Caller should clear memory and retry. "
                       << "If failures persist, migrate the workload and restart the host. requestId="
                       << importObj.returnReq.requestId;
    } else {
        importObj.status.state = UBSE_MEM_IMPORT_DESTROYED;
        EraseAddrImport(importObj);
    }
                                               ...
                                               UbseMemAddrBorrowImportObj& importObj)
{
    ...
    if (importObj.status.state == UBSE_MEM_IMPORT_SUCCESS) { // 导入成功
        ...
        if (ret != UBSE_OK) {
            BorrowFailedAdvice({MemFault::BORROW_MASTER_TO_REQ_SEND_FAILED, name, MemType::ADDR,
                                GetAddrReqSize(importObj.req), exportNodeId, importNodeId,
                                importObj.req.requestNodeId});
        }
        return ret;
    }
    UBSE_LOG_INFO << "[MMC] import failed, start rollback, requestId=" << importObj.req.requestId;
    // 导入失败 开始回滚
    ...
    if (importObj.status.state == UBSE_MEM_IMPORT_DESTROYED) {
        ...
        if (exportObjPtr) {
            ...
            if (auto ret = SendAddrExportObj(exportObj, true, exportNodeId); ret != UBSE_OK) {
                ...
                BorrowFailedAdvice({MemFault::RETURN_MASTER_TO_EX_SEND_FAILED, name, MemType::ADDR, 0, exportNodeId,
                                    req.requestNodeId, req.requestNodeId});
                return DealSendAddrUnExportObjFailed(resp, name, exportObj);
            }
            return UBSE_OK;
        } else {
            UBSE_LOG_INFO << "Success to delete single export, requestId=" << importObj.returnReq.requestId;
            resp.requestNodeId = req.requestNodeId;
            auto ret = BuildOperationRespWhenSuccess(resp, UBSE_OK, MemOperationType::ADDR_RETURN);
            if (ret != UBSE_OK) {
                BorrowFailedAdvice({MemFault::RETURN_MASTER_TO_REQ_SEND_FAILED, name, MemType::ADDR, 0, exportNodeId,
                                    req.requestNodeId, req.requestNodeId});
            }
            return ret;
        }
...
uint32_t HandlerDestroyExport(UbseMemAddrBorrowExportObj &exportObj, const UbseMemReturnReq &req,
                              UbseMemOperationResp &resp)
{
    ...
    if (exportObj.algoResult.exportNumaInfos.empty()) {
        return BuildOperationRespWhenFail(resp, req.name, req.requestNodeId, "exportNumaInfos is empty.",
                                          UBSE_ERR_INTERNAL, MemOperationType::ADDR_RETURN);
    }
    ...
    if (auto ret = SendAddrExportObj(exportObj, true, exportNodeId); ret != UBSE_OK) {
        BorrowFailedAdvice({MemFault::RETURN_MASTER_TO_EX_SEND_FAILED, name, MemType::ADDR, 0,
                            exportNodeId, req.importNodeId, requestNodeId});
                               ...
                               bool hasExport, const UbseMemReturnReq& req, UbseMemOperationResp& resp)
{
    ...
    if (importObj.status.state == UBSE_MEM_IMPORT_DESTROYED) {
        ...
        return HandlerDestroyExport(exportObj, req, resp);
    }
    ...
    if (ret != UBSE_OK) {
        BorrowFailedAdvice({MemFault::RETURN_MASTER_TO_IM_SEND_FAILED, name, MemType::ADDR, 0,
...
bool CheckReturnNodeValid(std::string importNodeId, std::string exportNodeId, std::string realRequestNodeId)
{
    return (importNodeId == realRequestNodeId) || (exportNodeId == realRequestNodeId);
}
                           ...
                           const std::string &realRequestNodeId)
{
    ...
    if (!IsMemBorrowFeatureSupported()) {
        BorrowFailedAdvice(
            {MemFault::RETURN_CHIP_NOT_SUPPORTED, req.name, MemType::ADDR, 0, "", req.importNodeId, req.requestNodeId});
    ...
    if (memStage == UbseMemStage::UBSE_CREATING || memStage == UbseMemStage::UBSE_DELETING) {
        UBSE_LOG_INFO << "resource is being borrowed or returned, name=" << req.name << ", requestId=" << req.requestId;
        auto ret = (memStage == UbseMemStage::UBSE_CREATING) ? UBSE_ERR_CREATING : UBSE_ERR_DELETING;
        BorrowFailedAdvice(
            {MemFault::RETURN_REQ_CONFLICT, req.name, MemType::ADDR, 0, "", req.importNodeId, req.requestNodeId});
        return ret;
    }
    ...
    auto exportNodeId = status.hasExport ? exportObj.req.exportNodeId : importObj.req.exportNodeId;
    if (!CheckReturnNodeValid(importNodeId, exportNodeId, realRequestNodeId)) {
