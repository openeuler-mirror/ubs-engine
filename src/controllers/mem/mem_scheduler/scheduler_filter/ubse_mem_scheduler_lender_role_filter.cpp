...
#include "ubse_mem_scheduler_lender_role_filter.h"

namespace ubse::mem::scheduler {

UbseResult LenderRoleFilter::FilterNodes(std::vector<NodeInfo>& nodes, const SchedulerNodeManager& nodeInfo,
                                         const SchedulerAccountManager& account, const SchedulerRequest& request)
{
    EraseNodesIf(nodes, [&](const NodeInfo& node) {
        ...
        if (!memNode->IsLender()) {
            RecordReject(node.nodeId, "node is not a lender");
            return true;
        }
        return false;
    });
