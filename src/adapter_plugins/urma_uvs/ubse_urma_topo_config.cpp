...
UbseUrmaTopoMode GetUrmaTopoMode()
{
    ...
    if (ret != UBSE_OK) {
        UBSE_LOG_WARN << "Failed to get URMA topo_mode config, use default non-cross, ret=" << FormatRetCode(ret);
        return UbseUrmaTopoMode::NON_CROSS;
    }
...
UbseResult LoadUrmaTopoConfig(UbseUrmaTopoMode topoMode, UbseUrmaTopoConfig& topoConfig)
{
    ...
    const std::string expectedLinkType = topoMode == UbseUrmaTopoMode::HCCS_CROSS ? URMA_TOPO_MODE_HCCS_CROSS :
                                                                                    URMA_TOPO_MODE_NON_CROSS;
