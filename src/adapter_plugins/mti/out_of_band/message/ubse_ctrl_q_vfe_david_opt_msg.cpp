...
UbseResult WriteReqMsg(CtrlQReqMessage &msg, const std::vector<UbseMtiIdevVfeDavidPair> &vfeDavidList)
{
    ...
    try {
        if (vfeDavidList.size() > UINT8_MAX) {
            UBSE_LOG_ERROR << "vfeDavidList size " << vfeDavidList.size() << " exceeds uint8_t max";
            return UBSE_ERROR;
        }
        writeHelper.Write<uint8_t>(static_cast<uint8_t>(vfeDavidList.size()));
