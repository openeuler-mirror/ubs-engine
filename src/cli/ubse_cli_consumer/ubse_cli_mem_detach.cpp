...
class UbseCliMemDetach::UbseCliMemDetachImpl {
public:
    UbseCliMemDetachImpl() {}
    ...
    std::shared_ptr<framework::UbseCliResultEcho> UbseCliDetachMem(const std::string& name)
    {
        ...
        UbseCliWaitIndicator waitIndicator("Detaching memory");
        uint32_t ret = ubse_invoke_call(UBSE_MEM, UBSE_MEM_CLI_SHM_DETACH, &reqBuffer, &resBuffer);
        UbseCliBufferGuard cliBufferGuard(resBuffer);
        if (ret == UBSE_ERR_TIMED_OUT) {
            return query.UbseCliGetShmMemByName(name, UbseCliShmOperation::DETACH);
        } else if (ret != UBSE_OK) {
            return UbseCliRegModule::UbseCliStringPromptReply(GetErrorMessage(ret));
        }
        // 成功回显
        return UbseCliRegModule::UbseCliStringPromptReply("Detach successfully");
    }
...
std::shared_ptr<framework::UbseCliResultEcho> UbseCliMemDetach::UbseCliDetachMem(const std::string& name)
{
    if (this->pImpl_ == nullptr) {
        return UbseCliRegModule::UbseCliStringPromptReply(systemd::error::ALLOCATION_ERROR);
    }
    return this->pImpl_->UbseCliDetachMem(name);
}

UbseCliMemDetach::UbseCliMemDetach()
{
    pImpl_ = SafeMakeUnique<UbseCliMemDetachImpl>();
}
UbseCliMemDetach::~UbseCliMemDetach() noexcept = default;
} // namespace ubse::cli::reg