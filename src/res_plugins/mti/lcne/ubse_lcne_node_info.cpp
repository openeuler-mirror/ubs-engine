...
UbseResult UbseLcneNodeInfo::QueryAllLcneIODieInfo(UbseLcneIODieInfoMap &ubseLcneIODieInfoMap)
{
    ...
    if (res != UBSE_OK) {
        UBSE_LOG_ERROR << "[MTI] Access the LCNE IO DIE information interface via HTTP is failed. "
                        << FormatRetCode(res);
        return res;
    }
    if (rsp.status != static_cast<int>(UbseHttpStatusCode::UBSE_HTTP_STATUS_CODE_OK)) {
        UBSE_LOG_ERROR << "[MTI] Access the LCNE IO DIE information interface is failed.The HTTP status code is "
                     << rsp.status;
        return UBSE_ERROR;
    }
    if (rsp.body.empty()) {
...
std::string UbseLcneIODieInfoMapToString(const UbseLcneIODieInfoMap &devMap)
{
    ...
    for (const auto &[devName, info] : devMap) {
        oss << "{" << "Device: " << devName.devName << ", ubControllerEid: " << info.ubControllerEid
            ...
            << ", chipType: " << static_cast<int>(info.chipType)
            << ", chipStatus: " << static_cast<int>(info.chipStatus) << "} ";
        oss << "\n";
    }
