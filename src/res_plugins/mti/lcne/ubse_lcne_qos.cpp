...
UbseResult UbseLcneQos::CreatQosProfile(UbseLcneQosProfile ubseLcneQosProfile)
{
    ...
    ret = UbseHttpModule::HttpSend(host, port, req, rsp);
    if (ret != UBSE_OK) {
...
UbseResult UbseLcneQos::DeleteQosProfile(std::string proflieName)
{
    UbseHttpRequest req;
    UbseHttpResponse rsp;

    req.method = "DELET";
    req.path = LCNE_QOS_PROFILE_URI + "/tqos-profile=" + proflieName;
    req.headers.emplace("Accept", LCNE_QOS_ACCEPT);
    req.headers.emplace("Content-Type", LCNE_QOS_CONTENT_TYPE);
...
UbseResult UbseLcneQos::ApplyVfeQos(UbseLcneFeInfo ubseFeInfo, std::string proflieName)
{
    ...
    req.headers.emplace("Accept", LCNE_QOS_ACCEPT);
    req.headers.emplace("Content-Type", LCNE_QOS_CONTENT_TYPE);
    auto ret = BuildQoSXml(ubseFeInfo, proflieName, body);
    if (ret != UBSE_OK) {
        UBSE_LOG_ERROR << "[MTI] buildQoSProfileXml failed. ";
        return ret;
    }
...
UbseResult UbseLcneQos::QueryVfeQos(UbseLcneFeInfo ubseFeInfo, std::string &proflieName)
{
    ...
    // 解析xml响应
    ret = ParseVfeQosResponse(rsp.body, proflieName);
    if (ret != UBSE_OK) {
...
UbseResult UbseLcneQos::ParseVfeQosResponse(std::string body, std::string &proflieName)
{
    std::shared_ptr<UbseXml> ubseXml = SafeMakeShared<UbseXml>(body);
    if (ubseXml == nullptr) {
