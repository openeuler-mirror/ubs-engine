...
/* 默认内存池中已使用节点对应的handle, 即普通内存借入及从动态预引入内存段的内存借入、以及指定HPA内存借入对应的handle */
#define UB_MEMORY_HANDLE_DEFAULT_USED_NODE 0

/* 动态预引入对应的handle */
#define UB_MEMORY_HANDLE_DYNAMIC_NODE 1

/* 静态预引入中已使用节点对应的handle,即从静态预引入内存段的内存借入对应的handle */
#define UB_MEMORY_HANDLE_STATIC_USED_NODE 2

/* 静态预引入对应的handle */
#define UB_MEMORY_HANDLE_STATIC_MEM_POOL 3

/* 含以上四种类型的全部handle */
#define UB_MEMORY_HANDLE_ALL_TYPE 4
